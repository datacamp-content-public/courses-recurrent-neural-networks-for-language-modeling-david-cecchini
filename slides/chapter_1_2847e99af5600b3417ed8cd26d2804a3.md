...
To train RNN models with LSTM cells can be very time consuming. To take into perspective, the training of 5 epochs on a core i7-8550U processor with 12GB of RAM memory took around 6 minutes, while a simple ANN model with Embedding and Dense layers took only 1.5 minutes to train.
The RNN model model achieved ~82% accuracy on the test data which is a good start.
...
`@script`
Now that we learned how to build a RNN model with keras, it is time to practice!
